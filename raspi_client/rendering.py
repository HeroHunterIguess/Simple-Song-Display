...
def render_standard(screen, c_s): # c_s is current_song
    ...
    try:
        ...
        if c.output:
            log_output("Album cover failed to load and render")
...
def render_centered(screen, c_s): # c_s is current_song
    ...
    try:
        ...
        if c.output:
            log_output("Failed to get background image:", err)
        return
    ...
    if c_s.is_playing == "False":
        ...
        try:
            ...
            if c.output:
                log_output("Failed to get or load pause icon:", err)
    ...
    if c.no_media_background_image_link != "":
        if cached_no_media_background is None:
            try:
                ...
                if c.output:
                    log_output("Failed to get no-media background:", err)
